...
def process_with_ai(text: str) -> dict:
    ...
    3. Премиальное форматирование (Используй возможности Notion-flavored Markdown):
        ...
        - Выделения (Callouts): Для сверхважных мыслей, сносок или инсайтов используй XML-теги <callout emoji="💡">Важная сноска</callout>. Они станут профессиональными блоками Notion Callout.
        - Сворачиваемый текст (Toggles): Для длинных списков или второстепенных подробностей используй теги <details><summary>Заголовок спойлера</summary>Скрытый текст внутри</details>. Внутри тега <summary> используй только ЧИСТЫЙ ТЕКСТ без HTML-тегов вроде <b> или <i> (пиши <summary>Технические шаги</summary>).
...
def polish_content(old_content: str, new_content: str) -> str:
    ...
    - Улучши форматирование, используя возможности Notion-flavored Markdown:
      ...
      * Спойлеры: используй `<details><summary>Чистый текст</summary>Контент</details>` (внутри `<summary>` пиши исключительно чистый текст без тегов вроде `<b>` или `<i>`).
      * Сноски: используй `<callout emoji="💡">Текст</callout>`.
